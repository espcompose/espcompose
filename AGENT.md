...
| `packages/esphome-target` | `@espcompose/esphome-target` | ESPHome backend — YAML config generation, C++ reactive runtime headers (`espcompose_bindings.h`, `espcompose_reactive.h`), `ExprNode` → C++ lowering, HA sensor injection, asset pipeline |
| `packages/simulator-target` | `@espcompose/simulator-target` | Browser preview backend — walks Semantic IR to produce an interactive HTML page with canvas-rendered LVGL widgets, JS reactive runtime (Signal/Memo/Scheduler), mock HA entity provider |
...
2. `useTheme()` returns a deep Proxy where leaf access creates cached
...
- **Never edit generated files directly** — Files under `packages/core/src/generated/`
  are auto-generated by `pnpm codegen` (via `packages/esphome-codegen/src/`). To change
