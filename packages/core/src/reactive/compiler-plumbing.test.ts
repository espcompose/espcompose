    ...
    it('collects dependencies from all signal arguments', () => {
      withReactiveScope(() => {
        const sig1 = new IRReactiveNode({
          kind: 'expression',
          dependencies: [
            { kind: 'dependency', sourceType: 'ha_entity', sourceId: 's1', sourceDomain: 'sensor' },
          ],
          exprType: 'float',
        });
        ...
        const sig2 = new IRReactiveNode({
          kind: 'expression',
          dependencies: [
            { kind: 'dependency', sourceType: 'ha_entity', sourceId: 's2', sourceDomain: 'binary_sensor' },
            { kind: 'dependency', sourceType: 'ha_entity', sourceId: 's3', sourceDomain: 'sensor' },
          ],
          exprType: 'bool',
        });
