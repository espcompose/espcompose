  ...
  describe('construction', () => {
    it('creates a node with dependencies', () => {
      const node = new IRReactiveNode({
        kind: 'memo',
        dependencies: [
          { kind: 'dependency', sourceType: 'ha_entity', sourceId: 'sensor_1', sourceDomain: 'sensor' },
          { kind: 'dependency', sourceType: 'ha_entity', sourceId: 'sensor_2', sourceDomain: 'binary_sensor' },
        ],
        exprType: 'float',
      });
    ...
    it('creates a single-source expression node', () => {
      const node = new IRReactiveNode({
        kind: 'expression',
        dependencies: [{ kind: 'dependency', sourceType: 'ha_entity', sourceId: 'ha_light_x', sourceDomain: 'binary_sensor' }],
        exprType: 'bool',
        sourceId: 'ha_light_x',
        propertyKey: 'state',
    ...
    it('isSingleSource = true for one dependency', () => {
      const node = new IRReactiveNode({
        kind: 'expression',
        dependencies: [{ kind: 'dependency', sourceType: 'ha_entity', sourceId: 'x', sourceDomain: 'sensor' }],
      });
    ...
    it('isSingleSource = false for multiple dependencies', () => {
      const node = new IRReactiveNode({
        kind: 'memo',
        dependencies: [
          { kind: 'dependency', sourceType: 'ha_entity', sourceId: 'a', sourceDomain: 'sensor' },
          { kind: 'dependency', sourceType: 'ha_entity', sourceId: 'b', sourceDomain: 'sensor' },
        ],
        exprType: 'float',
      });
